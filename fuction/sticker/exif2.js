...
// fluent-ffmpeg no incluye el ejecutable. En Termux/VPS usamos el ffmpeg
// instalado en el sistema y permitimos FFmpeg_PATH/FFMPEG_PATH como override.
try {
    const ffmpegPath = process.env.FFMPEG_PATH || process.env.FFmpeg_PATH || execFileSync('sh', ['-lc', 'command -v ffmpeg'], { encoding: 'utf8' }).trim()
    if (ffmpegPath) ff.setFfmpegPath(ffmpegPath)
} catch (_) {}
...
ff(tmpFileIn)
.on("error", err => reject(new Error(`No se pudo convertir a WebP: ${err?.message || err}. Comprueba que ffmpeg esté instalado.`)))
.on("end", () => resolve(true))
.addOutputOptions([
"-vcodec",
"libwebp",
...
async function writeExifImg2 (media, metadata) {
    ...
    if (metadata.packname || metadata.author) {
        ...
        const json = { "sticker-pack-id": `AkameBot-MD`, "sticker-pack-name": metadata.packname, "sticker-pack-publisher": metadata.author, "emojis": metadata.categories ? metadata.categories : [""] }
